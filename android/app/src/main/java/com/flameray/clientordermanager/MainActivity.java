...
public class MainActivity extends Activity {
    ...
    private void showPaymentDialog() {
        ...
        final Spinner orderSpin = spinner(form, "\u5173\u8054\u8BA2\u5355 *", labels.toArray(new String[0]), labels.get(0));
    ...
    private void confirmDeleteClient(final JSONObject client) {
        for (JSONObject o : objects(orders)) if (client.optString("id").equals(o.optString("clientId"))) { toast("\u8BE5\u5BA2\u6237\u5DF2\u6709\u8BA2\u5355\uFF0C\u4E0D\u80FD\u5220\u9664"); return; }
        new AlertDialog.Builder(this).setTitle("\u5220\u9664\u5BA2\u6237").setMessage("\u786E\u5B9A\u5220\u9664\u201C" + client.optString("name") + "\u201D\u5417\uFF1F").setNegativeButton("\u53D6\u6D88", null).setPositiveButton("\u5220\u9664", (d, w) -> { clients = without(clients, client.optString("id")); save(); showPage("\u5BA2\u6237"); }).show();
    }

    private void confirmDeleteOrder(final JSONObject order) {
        new AlertDialog.Builder(this).setTitle("\u5220\u9664\u8BA2\u5355").setMessage("\u8BA2\u5355\u548C\u5173\u8054\u7684\u6536\u6B3E\u8BB0\u5F55\u90FD\u4F1A\u5220\u9664\u3002").setNegativeButton("\u53D6\u6D88", null).setPositiveButton("\u5220\u9664", (d, w) -> {
            orders = without(orders, order.optString("id"));
            JSONArray next = new JSONArray(); for (JSONObject p : objects(payments)) if (!order.optString("id").equals(p.optString("orderId"))) next.put(p); payments = next;
            save(); showPage("\u8BA2\u5355");
        }).show();
    }

    private void confirmDeletePayment(final JSONObject payment) {
        new AlertDialog.Builder(this).setTitle("\u5220\u9664\u6536\u6B3E").setMessage("\u786E\u5B9A\u5220\u9664\u8FD9\u6761\u6536\u6B3E\u8BB0\u5F55\u5417\uFF1F").setNegativeButton("\u53D6\u6D88", null).setPositiveButton("\u5220\u9664", (d, w) -> {
            JSONObject order = find(orders, payment.optString("orderId")); if (order != null) try { order.put("paid", Math.max(0, order.optDouble("paid") - payment.optDouble("amount"))); } catch (Exception ignored) { }
            payments = without(payments, payment.optString("id")); save(); showPage("\u6536\u6B3E");
        }).show();
    }

    private void exportXlsx() { createDocument("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "\u5BA2\u6237\u8BA2\u5355\u7BA1\u7406\u5668_1.0.xlsx", REQUEST_EXPORT_XLSX); }
    private void exportBackup() { createDocument("application/json", "\u5BA2\u6237\u8BA2\u5355\u5907\u4EFD_" + today() + ".json", REQUEST_EXPORT_JSON); }
    private void importBackup() {
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT); intent.addCategory(Intent.CATEGORY_OPENABLE); intent.setType("application/json"); startActivityForResult(intent, REQUEST_IMPORT_JSON);
    }

    private void createDocument(String type, String name, int code) {
        Intent intent = new Intent(Intent.ACTION_CREATE_DOCUMENT); intent.addCategory(Intent.CATEGORY_OPENABLE); intent.setType(type); intent.putExtra(Intent.EXTRA_TITLE, name); startActivityForResult(intent, code);
    }
    ...
    @Override @Deprecated protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        try {
            if (requestCode == REQUEST_EXPORT_XLSX) { writeXlsx(uri); status("Excel \u8868\u683C\u5DF2\u5BFC\u51FA"); toast("Excel \u8868\u683C\u5DF2\u5BFC\u51FA"); }
            if (requestCode == REQUEST_EXPORT_JSON) { writeBackup(uri); status("\u5907\u4EFD\u5DF2\u5BFC\u51FA"); }
            if (requestCode == REQUEST_IMPORT_JSON) { readBackup(uri); status("\u5907\u4EFD\u5DF2\u5BFC\u5165"); showPage(currentPage); }
        } catch (Exception e) { toast("\u6587\u4EF6\u64CD\u4F5C\u5931\u8D25\uFF1A" + e.getMessage()); }
    }

    private void writeBackup(Uri uri) throws Exception {
        JSONObject backup = new JSONObject(); backup.put("clients", clients); backup.put("orders", orders); backup.put("payments", payments);
        try (OutputStream out = getContentResolver().openOutputStream(uri)) { out.write(backup.toString(2).getBytes(StandardCharsets.UTF_8)); }
    }

    private void readBackup(Uri uri) throws Exception {
        ByteArrayOutputStream buffer = new ByteArrayOutputStream();
        try (InputStream in = getContentResolver().openInputStream(uri)) { byte[] bytes = new byte[4096]; int n; while ((n = in.read(bytes)) > 0) buffer.write(bytes, 0, n); }
        JSONObject backup = new JSONObject(buffer.toString("UTF-8"));
        if (backup.optJSONArray("clients") == null || backup.optJSONArray("orders") == null || backup.optJSONArray("payments") == null) throw new Exception("\u5907\u4EFD\u683C\u5F0F\u65E0\u6548");
        clients = backup.getJSONArray("clients"); orders = backup.getJSONArray("orders"); payments = backup.getJSONArray("payments"); save();
    }

    private void writeXlsx(Uri uri) throws Exception {
        List<String[]> customerRows = new ArrayList<>(); customerRows.add(new String[]{"\u5BA2\u6237\u540D\u79F0", "\u8054\u7CFB\u65B9\u5F0F", "\u6765\u6E90", "\u72B6\u6001", "\u5907\u6CE8", "\u8BA2\u5355\u6570"});
        ...
        List<String[]> orderRows = new ArrayList<>(); orderRows.add(new String[]{"\u9879\u76EE/\u670D\u52A1", "\u5BA2\u6237", "\u8BA2\u5355\u91D1\u989D", "\u5DF2\u6536\u91D1\u989D", "\u5F85\u6536\u91D1\u989D", "\u622A\u6B62\u65E5\u671F", "\u72B6\u6001", "\u5907\u6CE8"});
        for (JSONObject o : objects(orders)) { JSONObject c = find(clients, o.optString("clientId")); double amount = o.optDouble("amount"), paid = o.optDouble("paid"); orderRows.add(new String[]{o.optString("title"), c == null ? "\u5DF2\u5220\u9664\u5BA2\u6237" : c.optString("name"), decimal(amount), decimal(paid), decimal(amount-paid), o.optString("due"), o.optString("status"), o.optString("note")}); }
        List<String[]> paymentRows = new ArrayList<>(); paymentRows.add(new String[]{"\u6536\u6B3E\u65E5\u671F", "\u8BA2\u5355", "\u5BA2\u6237", "\u6536\u6B3E\u91D1\u989D", "\u5907\u6CE8"});
